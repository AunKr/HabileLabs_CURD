...
exports.singUp = (req, res) => {
	...
	User.findOne({ email: email })
		.then((userData) => {
			if (userData) {
				/* if the email is found then user is already registered */
				res.json({
				...
				bcrypt
					...
					.then((hashedPassword) => {
						const user = new User({
							/* creating a new instance of User Schema and addiing the details of user for registeration */
					...
					.then((passwordIsValid) => {
						if (passwordIsValid) {
							const token = jwt.sign(
								{
									/* if password is valid then assign the token */
									email: user.email,
								},
								secret,
								{
